...
utEngine.prototype = {
    ...
    run : function() {
		...
		var localSelf = this;
		var runFunc = function() { localSelf.run(); };
		
        while (this.testNameIndex < this.testNames.length) {
            var index = this.testNameIndex;
            var testName = this.testNames[this.testNameIndex++];
            var testFunc = this.tests[testName];
...
asserter.prototype = {
    ...
    shouldBeSameAs : function(arg) {
        if (this.target != arg) {
            throw "Expected: exactly " + arg + ", " + this.valueName + ": " + this.target;
        }
    },
    
    shouldNotBeSameAs : function(arg) {
        if (this.target == arg) {
            throw "Expected: exactly " + arg + ", " + this.valueName + ": " + this.target;
        }
    },
	...
	shouldBeSameArrayAs : function(array) {
		if (isArray(this.target)) {
			if (this.target.length !== array.length) {
				throw this.valueName + " has wrong array length. Expected: " + array.length + ", actual: " + this.target.length;
			}
			
			array.forEach((function(item, index) {
				if (this.target[index] != item) {
					throw this.valueName + " mismatches at element " + index + ". Expected: " + item + ", actual: " + this.target[index];
				}
			}).bind(this));
		} else {
			if (this.target.byteLength !== array.length) {
				throw this.valueName + " has wrong array length. Expected: " + array.length + ", actual: " + this.target.byteLength;
			}
			
			var view = new Uint8Array(this.target);
			
			array.forEach((function(item, index) {
				if (view[index] != item) {
					throw this.valueName + " mismatches at element " + index + ". Expected: " + item + ", actual: " + view[index];
				}
			}).bind(this));
		}
	}
